...
router.post('/', (req, res) => {
    ...
    const sql = "INSERT INTO spare_parts (department, type, item_name, quantity) VALUES (?, ?, ?, ?)";
...
router.put('/:id', (req, res) => {
    ...
    const sql = "UPDATE spare_parts SET department=?, type=?, item_name=?, quantity=? WHERE id=?";
...
router.delete('/:id', (req, res) => {
    ...
    const sql = "DELETE FROM spare_parts WHERE id=?";
    db.query(sql, [id], (err) => {
        if (err) return res.status(500).json({ error: err.message });
